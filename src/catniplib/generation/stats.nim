...
proc setTerminal*(stats: var Stats, rawstat: TomlValueRef) =
    if rawstat == nil: # Set username to empty stat
        stats.terminal = newStat("", "", "")
        return
    ...
    if l > stats.maxlen:
        stats.maxlen = l
    stats.terminal = newStat(rawstat["icon"].getStr(), rawstat["name"].getStr(), rawstat["color"].getStr())
